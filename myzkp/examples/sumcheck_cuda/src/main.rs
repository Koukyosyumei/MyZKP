...
impl<'a> SumCheckProver<'a> {
    ...
    pub fn prove(&self, num_variables: usize, max_degree: usize, polynomial_factors: &[MPolynomial<F>]) -> Result<FiatShamirTransformer, Box<dyn std::error::Error>> {
...
impl SumCheckVerifier {
    pub fn verify(
        ...
        let recovered_num_vars = bincode::deserialize::<usize>(&transcript.pull()[0])?;
        assert_eq!(recovered_num_vars, num_vars);
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut transcript = prover.prove(num_vars, max_degree, &factors)?;
    let is_valid = SumCheckVerifier::verify(max_degree, num_vars, &factors, h, &mut transcript)?;
    assert!(is_valid);
