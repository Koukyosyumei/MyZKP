...
use crate::modules::algebra::ntt::{
    fast_coset_divide, fast_coset_evaluate, fast_interpolate, fast_zerofier,
};
...
pub struct FastStarkProof<F: Field> {
    ...
    pub tzc_points: Vec<F>,
    pub tzc_paths: Vec<MerklePath>,
}

pub struct FastStark<F: Field> {
...
impl<F: Field> FastStark<F> {
    pub fn preprocess(&self) -> (Polynomial<F>, Vec<F>, Vec<u8>) {
        ...
        let transition_zerofier_codeword_serialized: Vec<_> = transition_zerofier_codeword
            .iter()
            .map(|c| bincode::serialize(c).expect("Serialization failed"))
            .collect();
        let transition_zerofier_root = Merkle::commit(&transition_zerofier_codeword_serialized);
    ...
    pub fn prove(
        ...
        trace: &mut Trace<F>,
        boundary: &Boundary<F>,
        transition_zerofier: &Polynomial<F>,
        transition_zerofier_codeword: &Vec<F>,
        transition_constraints: &TransitionConstraints<F>,
    ) -> FastStarkProof<F> {
        ...
        for s in 0..self.num_registers {
            boundary_quotient_codewords.push(fast_coset_evaluate(
        ...
        let mut tzc_points = Vec::new();
        let mut tzc_paths = Vec::new();
        let serialized_transition_zerofier_codeword: Vec<_> = transition_zerofier_codeword
            .iter()
            .map(|b| bincode::serialize(&b).expect("Serialization failed"))
            .collect();
        for i in &duplicated_indices {
            tzc_points.push(transition_zerofier_codeword[*i].clone());
            tzc_paths.push(Merkle::open(*i, &serialized_transition_zerofier_codeword));
        }
...
pub fn initialize_fast_stark_m128(
...
) -> FastStark<FiniteFieldElement<M128>> {
